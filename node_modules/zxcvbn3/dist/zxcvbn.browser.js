var zxcvbn = (function (exports) {
    'use strict';

    // generated by build_frequency_lists.ts
     ...
     */
    function get_slanted_adjacent_coords(x, y) {
     ...
     */
    function get_aligned_adjacent_coords(x, y) {
     ...
     */
    function build_graph(layout_str, slanted) {
    ...
    class Scoring {
        ...
        static log10(n) { return Math.log(n) / Math.log(10); }
        ; // IE doesn't support Math.log10 :(
        static log2(n) { return Math.log(n) / Math.log(2); }
        ;
        static factorial(n) {
    ...
    class Matching {
        ...
        ;
        static extend(lst, lst2) { return lst.push.apply(lst, lst2); }
        static translate(string, chr_map) { return (Array.from(string.split('')).map((chr) => chr_map[chr] || chr)).join(''); }
        static mod(n, m) { return ((n % m) + m) % m; } // mod impl that works for negative numbers
        ...
        static spatial_match(password, additionalGraphs) {
            ...
            for (const [name, { layout, slanted }] of Object.entries(additionalGraphs || {})) {
                if (layout) {
...
`);
    ...
    Matching.SHIFTED_RX = /[~!@#$%^&*()_+QWERTYUIOP{}|ASDFGHJKL:"ZXCVBNM<>?]/;
...
`,
    ...
    const zxcvbn = function (password, options) {
